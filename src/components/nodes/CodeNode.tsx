...
function CodeNode({ id, data, selected }: NodeProps<C4NodeData>) {
  const c = getNodeColors('code', data.customColor);
    ...
    >
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 font-sans text-xs leading-relaxed text-slate-500 line-clamp-2">{data.description}</p>
          {data.technology && (
            <span className={`mt-2 inline-block font-sans text-xs font-medium ${c.badge} ${c.badgeBg} px-2 py-1 rounded-md ring-1 ring-inset ${c.badgeRing}`}>
              {data.technology}
            </span>
          )}
        </div>
